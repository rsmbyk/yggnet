---
id: ITEM-003
status: archived
title: "Explore bird's-eye camera"
type: feat
priority: P0
effort: L
created: 2026-07-30
updated: 2026-07-30
spec: specs/003-explore-bird-s-eye-camera
branch:
pr:
archived_at: 2026-07-30
archive_reason: released
release_version: 0.21.1
---

# ITEM-003: Explore bird's-eye camera

## Summary

RTS/SimCity bird's-eye pan, zoom, light orbit; overview and street-level altitude.

## Notes

Not FPS/cockpit. Overview vs street is the same camera at different height. Related: ITEM-004, ITEM-007, ITEM-019, ITEM-020.

## Acceptance sketch

- Pan and zoom the overlook camera
- Limited tilt/orbit without leaving bird's-eye feel
- Toggle or gesture for overview vs street-level framing

## Links

- Spec: [SPEC-003](../../specs/003-explore-bird-s-eye-camera/spec.md)
- Related items: ITEM-004, ITEM-007, ITEM-019, ITEM-020.
